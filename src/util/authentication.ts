...
LoginRouter.post("/", async (req, res) => {
    if (!process.env.CAS_SERVICE_URL) {
        throw new Error("CAS Service URL is not configured");
    }
    // Validate the token passed by the client
    const response = await axios.get(
        "https://cas-auth.rpi.edu/cas/serviceValidate?" +
            "ticket=" +
            req.body.ticket +
            "&service=" +
    ...
    if (
        ...
        userNodes.length > 0 &&
        userNodes[0] &&
        userNodes[0].textContent
    ) {
        req.session.rcs_id = userNodes[0].textContent.toLowerCase();
        const user = await User.getUserFromEmail(
            req.session.rcs_id + "@rpi.edu"
        );
        req.session.admin = PermissionTools.isAdmin(user);
        return res.json({
            rcs_id: userNodes[0].textContent.toLowerCase(),
